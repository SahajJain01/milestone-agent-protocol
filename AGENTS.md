...
4. Read `.agent/client-log.md` only for external provenance.
5. Inspect `PROTOCOL.md`, `protocol.yaml`, and the smallest relevant template, migration, adoption,
   or adapter.

The protocol and metadata are authoritative for behavior; decisions explain why; todo owns unfinished
work; milestone lifecycle records own archived outcome contracts and closure history.
...
.agent/                             # this repository's own canonical context
  milestones/                       # indexed durable milestone lifecycle records
```
...
- Archive a verified milestone closure before removing it from the active todo. Keep lifecycle
  entries append-only and store completed task IDs rather than completed task bodies.
- Reopen a closed milestone only to restore or preserve its archived original end goal. Reuse its ID
  and outcome contract, append a dated evidence-backed reopening, and allocate new task IDs. Import a
  pre-archive closure from Git only when stable history is unambiguous; otherwise fail closed. Create
  a new milestone when the end goal changes materially.
...
- `PROTOCOL.md`, `protocol.yaml`, templates, migrations, adoptions, milestone archive rules, and the
  skill agree.
