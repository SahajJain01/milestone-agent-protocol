---
id: legacy-unlocked-to-2.4.0
from: legacy-unlocked
to: 2.4.0
---

# Adopt a verified legacy-unlocked target into protocol 2.4.0
...
- `AGENTS.md`, `.agent/todo.md`, `.agent/client-log.md`, and `.agent/decisions/index.md` exist, are
  tracked, and are clean at the baseline commit.
- `.agent/milestones/index.md` and `.agent/milestones/M-###.md` are absent. Any pre-existing content
  in that namespace has ambiguous ownership and fails this adoption instead of being overwritten.
...
2. Reconcile declared target paths semantically to the 2.4.0 structural contract. Preserve target
   ...
   than guessing.
3. Create `.agent/milestones/index.md` with a forward-only coverage boundary at the adoption
   timestamp. State that earlier closures remain Git-only unless imported later from stable evidence;
   do not synthesize historical lifecycle records during adoption.
4. Record justified local divergences once. Do not claim earlier semantic migrations or copy source
   repository product details into the target.
5. Verify the complete target and its safe repository-specific gate. Confirm a second 2.4.0
   reconciliation would produce zero diff.
6. Write `.agent/protocol.lock.yaml` last with the validated source/version/digest, current managed
   paths and rules, `last_applied: 2.4.0`, `applied_migrations: []`, and
   `applied_adoptions: [legacy-unlocked-to-2.4.0]`.
7. Add a `legacy_adoption` block containing `specification`, `adopted_at`, `baseline_commit`,
...
- The archive index has an honest forward-only coverage boundary and no invented historical rows.
- Required paths, current rule identities, milestone/task/decision/archive schemas, immutable IDs,
  internal links, external-evidence anchors, and source anchors validate.
...
- A repeated 2.4.0 initialization uses the lock and produces zero diff.
